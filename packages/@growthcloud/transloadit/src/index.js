...
export default class Transloadit extends BasePlugin {
  ...
  /** @deprecated use `import { COMPANION_URL } from '@growthcloud/transloadit'` instead. */
  static COMPANION = COMPANION_URL;

  /** @deprecated use `import { COMPANION_ALLOWED_HOSTS } from '@growthcloud/transloadit'` instead. */
  ...
  constructor(uppy, opts) {
    ...
    this.opts.assemblyOptions =
      opts.assemblyOptions ?? this.opts.getAssemblyOptions;
    this.#rateLimitedQueue = new RateLimitedQueue(this.opts.limit);

    this.i18nInit();

    const hasCustomAssemblyOptions =
      this.opts.assemblyOptions !== defaultOptions.assemblyOptions;
    if (this.opts.params) {
  ...
  install() {
    ...
    if (this.opts.importFromUploadURLs) {
      ...
      this.uppy.use(Tus, {
        ...
        // Pass the limit option to @growthcloud/tus
