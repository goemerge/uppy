...
function getAllFilesPerPlugin(pluginNames) {
  ...
  function getFiles(name) {
    return glob
      .sync(`${root}/packages/@growthcloud/${name}/lib/**/*.js`)
    ...
    for (const key of Object.keys(locale.strings)) {
      ...
      if (!fileString.match(regPat)) {
        return Promise.reject(
          new Error(`Unused locale key "${key}" in @growthcloud/${name}`)
        );
      }
    }
  }
...
function test() {
  switch (mode) {
    case "unused":
      return getPaths(`${root}/packages/@growthcloud/**/src/locale.js`).then(
        (paths) =>
          unused(
    ...
    case "warnings":
      return getLocales(`${root}/packages/@growthcloud/locales/src/*.js`).then(
        (locales) =>
          warnings({
