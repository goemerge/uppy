...
await fs.mkdir(new URL("./@growthcloud/locales/dist", PACKAGES_ROOT), {
  recursive: true,
});
...
const localesModules = await fs.opendir(
  new URL("./@growthcloud/locales/src/", PACKAGES_ROOT)
);
for await (const dirent of localesModules) {
  if (!dirent.isDirectory() && dirent.name.endsWith(".js")) {
    ...
    methods.push(
      buildBundle(
        `./packages/@growthcloud/locales/src/${localeName}.js`,
        `./packages/@growthcloud/locales/dist/${localeName}.min.js`,
        { minify: true }
      )
    );
  }
