...
const localesRoot = path.join(__dirname, "../packages/@growthcloud/locales");
...
const packages = [
  ...
  "@growthcloud/url",
  "@growthcloud/webcam",
  "@growthcloud/xhr-upload",
  "@growthcloud/drop-target",
  // Stores
...
async function getMinifiedSize(pkg, name) {
  ...
  if (name !== "@growthcloud/core" && name !== "uppy") {
    // Preact is already unconditionally included through @growthcloud/core
    external.unshift("@growthcloud/core", "preact");
  }
...
async function injectSizes(config) {
  ...
  const sizesPromise = Promise.all(
    packages.map(async (pkg) => {
      ...
      console.info(
        chalk.green(
          // ✓ @growthcloud/pkgname:     10.0 kB min  / 2.0 kB gz
...
function injectLocaleList() {
  ...
  glob.sync(localePackagePath).forEach((localePath) => {
    ...
    const npmPath = `<code class="raw"><a href="https://www.npmjs.com/package/@growthcloud/locales">@growthcloud/locales</a>/lib/${localeName}</code>`;
    const cdnPath = `[\`${localeName}.min.js\`](https://releases.transloadit.com/uppy/locales/v${localePackageVersion}/${localeName}.min.js)`;
    const githubSource = `[\`${localeName}.js\`](https://github.com/goemerge/uppy/blob/main/packages/%40uppy/locales/src/${localeName}.js)`;
