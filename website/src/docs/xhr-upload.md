...
The `@growthcloud/xhr-upload` plugin handles classic HTML multipart form uploads, as well as uploads using the HTTP `PUT` method.
...
This plugin is published as the `@growthcloud/xhr-upload` package.

Install from NPM:

```shell
npm install @growthcloud/xhr-upload
```
...
The `@growthcloud/xhr-upload` plugin has the following configurable options:
