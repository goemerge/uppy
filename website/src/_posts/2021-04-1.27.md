...
In Uppy 1.27 we’ve added a new `@growthcloud/drop-target` plugin, enabled support for Vue 3, introduced dynamic `metaFields` option for Dashboard, Google Drive now lists documents that have been shared with you.
...
With `@growthcloud/drop-target` it is now possible to turn your whole app / page (or any other element, like `<textarea>`, for instance) into a drag-and-drop area:
...
[PR #2758](https://github.com/goemerge/uppy/pull/2758).
...
`@growthcloud/vue` integration now supports Vue 3! This is done by checking the arguments of the `render` function and determining whether it is using Vue 2 or 3, and then returning the appropriate rendering method.

[PR #2755](https://github.com/goemerge/uppy/pull/2755).

## Companion CORS and Better Testing

In Companion, we’ve improved unit testing DX ([#2827](https://github.com/goemerge/uppy/pull/2827)) and switched to using the `cors` module instead of custom cors logic ([#2823](https://github.com/goemerge/uppy/pull/2823)).

@coreprocess submited a fix that allows Companion to serve on a subpath ([#2797](https://github.com/goemerge/uppy/pull/2797)), and added Redis pubsub scope setting ([#2804](https://github.com/goemerge/uppy/pull/2804))!
...
[PR #2835](https://github.com/goemerge/uppy/pull/2835)
...
[PR #2796](https://github.com/goemerge/uppy/pull/2796)
...
See [changelog](https://github.com/goemerge/uppy/blob/master/CHANGELOG.md#1270) for details.
